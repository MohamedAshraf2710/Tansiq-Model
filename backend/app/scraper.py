...
def _scrape_direct_url(url: str) -> list[dict]:
    ...
    try:
        ...
        keywords = ["تنسيق", "أقسام", "شروط", "مصاريف", "قبول"]
        targeted_blocks = []
        fallback_blocks = []
        ...
        for p in paragraphs:
            ...
            if len(ptxt) > 20:
                fallback_blocks.append(ptxt)
                if any(kw in ptxt for kw in keywords):
                    targeted_blocks.append(ptxt)
                    
        if targeted_blocks:
            text_content = " ".join(targeted_blocks)[:2000]
        else:
            text_content = " ".join(fallback_blocks)[:2000]
        
        if text_content:
            return [{"text": text_content, "url": url, "source": "web_direct"}]
    except Exception as e:
    ...
    try:
        ...
        try:
            for item in soup.select("div.BNeawe, div.s3v9rd, div.kCrYT"):
                try:
                    text_content = item.get_text(strip=True).replace('\n', ' ').replace('\r', ' ')
                    ...
                    logger.warning("Item parse failed: %s", type(inner_e).__name__)
        except Exception as outer_e:
            logger.warning("Search scrape select loop failed: %s", type(outer_e).__name__)
    except Exception as e:
...
def smart_lookup(query: str) -> dict:
    ...
    # 2. Specific detail lookup for a direct URL
    faculty_detail = get_faculty_details(query)
    direct_url = None
    if faculty_detail:
        direct_url = faculty_detail.get("url")
        if not direct_url:
            logger.info("smart_lookup: Found faculty detail in DB (no URL)")
            return {"results": [faculty_detail], "source": "database"}

    # 3. Web fallback
    logger.info("smart_lookup: DB miss or checking URL — falling back to web scraping")
    web_results = scrape_web(query, direct_url=direct_url)
    if web_results:
